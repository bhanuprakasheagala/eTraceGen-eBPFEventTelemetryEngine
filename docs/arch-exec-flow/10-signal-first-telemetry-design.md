...
This document defines the next design phase for eTraceGen after the current capture-first v1 behavior. The objective is to move from a broad, low-level event stream to a focused, high-signal telemetry pipeline that looks more like enterprise security logging products such as Sysmon, Falco, and auditd.

Status: documentation-only design. No implementation changes are planned until this design is reviewed and approved.
