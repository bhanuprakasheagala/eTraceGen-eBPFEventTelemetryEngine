...
> **Build status:** the socket schema and kernel handlers (`bpf/event_logger_network.bpf.c`) are
> implemented, but the network module is **not yet compiled into the BPF object** and has not been
> validated end-to-end. Enabling and validating it is [roadmap.md](roadmap.md) **Milestone 2**
> (after the signal-first noise-reduction work). This document is the design reference for that work.
...
## 16. Known Coverage Limitations (to address when built in)
- **fd-tracking gap:** data-plane hooks (`read`/`write`/`readv`/`writev`/`sendmmsg`/`recvmmsg`) only
  fire for fds recorded in the socket-fd map, which is populated by `socket`/`accept`/`socketpair`
  observed after startup. Sockets opened before the collector started, or received via `SCM_RIGHTS`
  or `dup`, are invisible to these hooks. Mitigations (e.g. a startup fd-table scan) are future work.
- **Record size / volume:** `network_event` is large (~600 bytes, dominated by two 108-byte UNIX
  path buffers). With per-syscall data-plane hooks enabled this amplifies volume — exactly the
  noise pressure the knowledge/policy layers are meant to absorb (see [roadmap.md](roadmap.md)).
